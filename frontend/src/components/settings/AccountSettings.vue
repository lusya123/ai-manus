<template>
  <div class="space-y-6 w-full">
    ...
    <div class="space-y-6">
      ...
      <div v-if="canChangePassword" class="rounded-[12px] p-[12px] -m-[12px]">
        <div class="flex flex-wrap items-center justify-between gap-4">
          <div class="flex flex-col items-start justify-center flex-1 min-w-0">
            <span class="text-sm font-medium text-[var(--text-primary)]">{{ t('Password') }}</span>
            <div class="text-[13px] text-[var(--text-tertiary)] leading-[18px] w-full">
              {{ t('Change the password used to sign in to this account.') }}
            </div>
          </div>
          <button
            type="button"
            data-testid="open-change-password"
            class="inline-flex h-[32px] min-w-16 items-center justify-center gap-[6px] whitespace-nowrap rounded-[10px] bg-transparent px-[12px] text-sm font-medium text-[var(--text-primary)] outline outline-1 -outline-offset-1 outline-[var(--border-btn-main)] transition-colors hover:bg-[var(--fill-tsp-white-light)] hover:opacity-90 active:opacity-80"
            @click="changePasswordDialog?.open()"
          >
            {{ t('Update Password') }}
          </button>
        </div>
...
const changePasswordDialog = ref<InstanceType<typeof ChangePasswordDialog> | null>(null)
const canChangePassword = computed(() => authProvider.value === 'password')
...
const handlePasswordChanged = async () => {
  // The backend intentionally revokes all sessions after a password change.
  await logout(true)
